...
public class ButtonPanel_PvP extends JPanel{
    private RulesPanel_PvP rule;
    ...
    private PvP_Mode frame;
    private int playerNumber;

    public ButtonPanel_PvP(ArrayList<JLabel> inputLabelList, ArrayList<JLabel> resultLabelList, PvP_Mode frame, int[] answer, int pn)throws Exception{
        count = new int[] {0};
        this.answer = answer;
        ...
        newGame.addActionListener(
            new ActionListener(){
                public void actionPerformed(ActionEvent evt){
                    NewGameDialog ngd = new NewGameDialog(frame);
                }
            }
        );
        ...
        attack.addActionListener(
            new ActionListener(){
                public void actionPerformed(ActionEvent evt){
                    ...
                    if(eatBite[0] == 4){
                        GameFinishDialog gcd = new GameFinishDialog(frame);
                        disableButtons();
                    }else if(playerNumber == 2 && count[0] == 7){
                        DrawDialog god = new DrawDialog(frame);
                        disableButtons();
                    }else{
                        resetButton();
                        count[0]++;
    ...
    class GameFinishDialog extends JDialog{  //ゲームの勝敗を表示するダイアログ
        GameFinishDialog(JFrame owner){
            ...
            confirmation = new JLabel("<html>おめでとうございます！<br>player" + playerNumber +"の勝利です！<html>");
    ...
    class DrawDialog extends JDialog{   //8回以内にどちらも正解できなかった時に表示するダイアログ
        DrawDialog(JFrame owner){
            ...
            JLabel confirmation = new JLabel("引き分けです");
            confirmation.setBounds(25, 30, 250, 70);
            confirmation.setFont(new Font(Font.DIALOG_INPUT, Font.BOLD, 14));
            confirmation.setHorizontalAlignment(JLabel.CENTER);
            add(confirmation);
